defmodule Whistle do
  ...
  def run do
    {:ok, file} = File.open("whistle.dot", [:write])
    IO.binwrite(file, get_graph())
    File.close(file)
    System.cmd("sh", ["-c", "cd  /Users/tomberman/Development/reqrecipe/; dot -Tpng whistle.dot > whistle.png"])
    System.cmd("open", ["whistle.png"])
  ...
  def calc_branches(l) do
    ...
    l["actions"]
    |> Enum.map(fn(action) -> "\"#{ref}\" -> \"#{get_title_from_ref(action["details"]["to"]["value"])}\"" end)
  end
  ...
  def build_fields() do
    ...
    fields
    |> Enum.map(fn(field) -> field["title"] end)
  end

  def get_links do
    f = build_fields()
    scan_l  = length(f) - 1
    Enum.scan(0..scan_l, '',fn x, acc -> "\"#{Enum.at(f, x)}\" -> \"#{Enum.at(f, x + 1)}\""  end)
  end
  ...
  def get_graph() do
    ...
    nodes = build_fields() |> Enum.reduce("", fn x, acc -> "#{acc} \"#{x}\"" end)
    links = get_links()
    b = branches()
    "#{t} #{nodes} #{links} #{b} #{e}"
  end
